...
class PatchEmbeddings(nn.Module):
    ...
    patch into a embedding vector via a single Conv2d layer.
    """
    ...
    def __init__(self, config) -> None:
        ...
        ----------
        config : object
            Must expose ``num_channels``, ``hidden_size``,
            and ``patch_size`` attributes.
        """
        super().__init__()
        
        # Conv2d with kernel_size == stride == patch_size acts as
        # a non-overlapping sliding window that extracts one
        # patch per spatial position and linearly projects it
        # to hidden_size in a single operation — equivalent to
        # slicing the image into patches and applying nn.Linear
        # to each flattened patch, but more memory-efficient.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        torch.Tensor
            ...
            (batch, num_patches, hidden_size) where
        ...
        # flatten spatial dims into a single sequence axis, then
        # transpose so the sequence length precedes the channel
        # dim, matching the (batch, seq_len, hidden) layout
        # expected by transformer layers downstream.
